...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('TitleBack'));
		bg.screenCenter();
		...
		particles.frames = Paths.getSparrowAtlas('ara/verruckt/particles', 'shared');
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			new FlxTimer().start(2, function(tmr:FlxTimer)
			{
				...
				FlxG.switchState(new LanguageAdvisor());
			});
	...
	override function beatHit()
	{
		...
		switch (curBeat)
		{
			...
			// credTextShit.screenCenter();
			case 10:
				createCoolText(['Three Original SONGS']);
			// credTextShit.visible = true;
			case 11:
				addMoreText('Enjoy :)');
			// credTextShit.text += '\nlmao';
